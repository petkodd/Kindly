...
async function insertSubscription(
  ...
  await q.query(
    `INSERT INTO subscriptions (buyer_id, plan, status, stripe_sub_id, current_period_end)
     VALUES ($1, 'family', $2, $3, $4)`,
    [buyerId, opts.status, opts.stripeSubId ?? `sub_${Math.random()}`, opts.currentPeriodEnd],
  );
}
...
describe('subscriptionRepo.getForBuyer', () => {
  it('returns null when the buyer has never checked out', async () => {
    const buyer = await makeBuyer('sarah@example.com');
    expect(await subscriptionRepo.getForBuyer(q, buyer)).toBeNull();
  });

  it('returns the most recent row when several exist', async () => {
    const buyer = await makeBuyer('sarah@example.com');
    await insertSubscription(buyer, { status: 'canceled', currentPeriodEnd: new Date('2026-01-01') });
    await insertSubscription(buyer, { status: 'trialing', currentPeriodEnd: new Date('2026-07-01') });
    const sub = await subscriptionRepo.getForBuyer(q, buyer);
...
describe('subscriptionRepo.isBillingCurrent', () => {
  it('is false with no subscription at all', async () => {
    ...
    expect(await subscriptionRepo.isBillingCurrent(q, buyer)).toBe(false);
  });

  it('is true while trialing', async () => {
    const buyer = await makeBuyer('b@example.com');
    await insertSubscription(buyer, { status: 'trialing', currentPeriodEnd: new Date(Date.now() + 86400000) });
    expect(await subscriptionRepo.isBillingCurrent(q, buyer)).toBe(true);
  });

  it('is true while active', async () => {
    const buyer = await makeBuyer('c@example.com');
    await insertSubscription(buyer, { status: 'active', currentPeriodEnd: new Date(Date.now() + 86400000) });
    expect(await subscriptionRepo.isBillingCurrent(q, buyer)).toBe(true);
  });
  ...
  it('is true for past_due still within the 3-day grace window', async () => {
    ...
    await insertSubscription(buyer, { status: 'past_due', currentPeriodEnd: periodEnd });
    expect(await subscriptionRepo.isBillingCurrent(q, buyer)).toBe(true);
  });
  ...
  it('is false once canceled', async () => {
    ...
    await insertSubscription(buyer, { status: 'canceled', currentPeriodEnd: new Date(Date.now() + 86400000) });
    expect(await subscriptionRepo.isBillingCurrent(q, buyer)).toBe(false);
  });
});
...
describe('subscriptionRepo.upsertFromStripeSubscription', () => {
  ...
  it('maps unrecognized/terminal Stripe statuses to canceled', async () => {
    ...
    const sub = await subscriptionRepo.upsertFromStripeSubscription(q, {
      ...
      status: 'unpaid',
      current_period_end: periodEndUnix,
      metadata: { buyer_id: buyer },
    });
    expect(sub.status).toBe('canceled');
  });

  it('throws when metadata.buyer_id is missing on first sync', async () => {
    await expect(
      subscriptionRepo.upsertFromStripeSubscription(q, {
        id: 'sub_no_buyer',
